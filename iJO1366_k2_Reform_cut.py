...
def lazyctr(model,where): # cut according to the first formulation
    ...
    if where == GRB.Callback.MIPSOL:
        ...
        print('*** Begin Lazy Constraint ***')
        if abs(model._vij[bm_core]-model._voj[bm_core]) >= eps:

            print('**Input Values for lazy constraint **')
            ...
            model._vij[bm_core]*(gp.quicksum((1-model._varsy[j]) for j in keys if abs(model._vij[j]) != 0)))
...
m.addConstr(v[bm_core] >= .5*FBA_WT, name='expected_growth')
